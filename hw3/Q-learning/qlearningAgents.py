...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        # The case at the terminal state, you should return None.
        if len(LegalActions) == 0:
            return None
        ActionValues = Counter()
        bestQValue = -10000000000

        # Iterate all legal actions to find the maximum q-value
        for action in LegalActions:
            CurQValue = self.getQValue(state, action)
            ActionValues[action] = CurQValue
            if CurQValue > bestQValue:
                bestQValue = CurQValue

        bestActions = []
        for key,value in ActionValues.items():
            if value == bestQValue:
                bestActions.append(key)

        # if Qvalue has more than one maximum value, choose romdomly.
