...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        wcfs = [WeakClassifier(feature) for feature in features]

        bestClf = None
        bestError = float('inf') # 將besterror先設成無線大 因為等一下要取max

        for clf in wcfs: # 遍歷每個 classifer
            error = 0 # error 用來計算當前的classifer的 error weights
            for idx in range(len(iis)): # 遍歷每張圖片
                if clf.classify(iis[idx]) != labels[idx]: # 如果 classify回傳的跟原本的label不一樣代表他辨識錯誤
                    error += weights[idx]
            if error < bestError: # 拿 error 跟當前的 besterror 取最大值
                bestClf = clf
                bestError = error
